...
const AlertBanner = memo(function AlertBanner({ data }: { data: StatusData }) {
  ...
  return (
    <div role="alert" className="flex items-center gap-3 rounded-xl border border-destructive/25 bg-destructive/[0.04] px-4 py-3">
      <AlertTriangle className="h-4 w-4 shrink-0 text-destructive" />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-destructive">
          {tokenExpired ? t.dashboard.alertTokenExpired : t.dashboard.alertTokenExpiring}
        </p>
        <p className="text-xs text-destructive/80 mt-0.5">
          {tokenExpired
            ? t.dashboard.alertTokenExpiredDesc
            : interpolate(t.dashboard.alertTokenExpiringDesc, {
                time: formatDistanceToNow(new Date(data.token_expires_at), { addSuffix: true, locale: dateLocale }),
              })}
        </p>
      </div>
      <Button
        ...
        onClick={() => { window.location.href = "/auth/refresh"; }}
      >
...
const KpiCards = memo(function KpiCards({ data }: { data: StatusData }) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <StatusCard
        title={t.dashboard.microsoft365}
        value={data.user_email}
        description={tokenDescription}
        icon={<Shield className="h-4 w-4" />}
        variant={tokenExpired ? "destructive" : "default"}
        badge={tokenExpired ? t.common.expired : undefined}
      />
      <StatusCard
        title={t.dashboard.webhooksActive}
      ...
      />
      <StatusCard
        title={t.dashboard.briefings30d}
      ...
      <Link to="/memories" aria-label={`${t.dashboard.memories}: ${data.memories_count}`}>
        <StatusCard
          ...
          description={t.dashboard.viewManageMemories}
...
const EmbeddingsTable = memo(function EmbeddingsTable({ data }: { data: StatusData["embeddings_by_service"] }) {
  const { t } = useI18n();
  return (
      ...
      <CardContent className="pt-0">
        {data.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1">{t.dashboard.embeddingsAutoCreated}</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                <TableHead className="text-[11px] uppercase tracking-wider">{t.dashboard.tool}</TableHead>
                <TableHead className="text-right text-[11px] uppercase tracking-wider">{t.dashboard.total}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {data.map((e) => (
                <TableRow key={e.service}>
                  ...
                  <TableCell className="text-right tabular-nums text-muted-foreground">{e.count.toLocaleString()}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
const TokenUsageSection = memo(function TokenUsageSection({ data }: { data: StatusData["tokens_30d"] }) {
  const { t } = useI18n();
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          {total > 0 && (
            <span className="text-xs font-normal text-muted-foreground tabular-nums">
              {interpolate(t.dashboard.totalTokens, { count: total.toLocaleString() })}
            </span>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-0">
        {total === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1 max-w-[240px]">{t.dashboard.tokensExplain}</p>
          </div>
        ) : (
...
const McpActivitySection = memo(function McpActivitySection({ data }: { data: StatusData["mcp_activity_30d"] }) {
  const { t } = useI18n();
  return (
    <Card className="shadow-soft">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center justify-between">
          ...
          {data.total_calls > 0 && (
            <span className="text-xs font-normal text-muted-foreground tabular-nums">
              {interpolate(t.dashboard.calls, { count: data.total_calls })}
            </span>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-0">
        {data.total_calls === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1 max-w-[240px]">{t.dashboard.mcpExplain}</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg bg-accent/50 px-3 py-2 text-center">
                ...
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider">{t.dashboard.total}</p>
              </div>
              <div className="rounded-lg bg-accent/50 px-3 py-2 text-center">
                <p className="text-lg font-semibold tabular-nums text-emerald-500">{data.successful}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider">{t.dashboard.success}</p>
              </div>
              <div className="rounded-lg bg-accent/50 px-3 py-2 text-center">
                <p className="text-lg font-semibold tabular-nums text-destructive">{data.failed}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider">{t.dashboard.errors}</p>
              </div>
            </div>
            {data.tools_used.length > 0 && (
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead className="text-[11px] uppercase tracking-wider">{t.dashboard.tool}</TableHead>
                    <TableHead className="text-right text-[11px] uppercase tracking-wider">{t.dashboard.callsHeader}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {data.tools_used.map((tool) => (
                    <TableRow key={tool.service}>
                      <TableCell className="font-medium text-xs">{tool.service}</TableCell>
                      <TableCell className="text-right tabular-nums text-muted-foreground">{tool.count}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
...
const RecentBriefings = memo(function RecentBriefings({ briefings }: { briefings: StatusData["recent_briefings"] }) {
  const { t, locale } = useI18n();
  const dateLocale = locale === "pt" ? ptBR : enUS;
    ...
    <Card className="shadow-soft">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center justify-between">
          ...
          {briefings.length > 0 && (
            <Link to="/briefings" className="text-xs font-medium text-brand hover:text-brand/80 transition-colors flex items-center gap-1">
              {t.common.viewAll}
      ...
      <CardContent className="pt-0">
        {displayed.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1">{t.dashboard.briefingsAutoGenerated}</p>
          </div>
        ) : (
          <ul className="divide-y divide-border -mx-2" aria-label={t.dashboard.recentBriefings}>
            {displayed.map((b) => (
              <li key={b.event_id}>
                <Link
                ...
                >
                  <span className="font-medium truncate flex-1">{b.event_subject}</span>
                  <span className="text-xs text-muted-foreground tabular-nums shrink-0">
                    {format(new Date(b.event_start), locale === "pt" ? "dd 'de' MMM '·' HH:mm" : "MMM dd '·' HH:mm", { locale: dateLocale })}
                  </span>
        ...
        {hasMore && (
          <div className="mt-3 text-center">
            <Link to="/briefings" className="text-xs font-medium text-brand hover:text-brand/80 transition-colors">
              {interpolate(t.dashboard.moreBriefings, { count: briefings.length - MAX_RECENT_BRIEFINGS })}
            </Link>
          </div>
        )}
      </CardContent>
